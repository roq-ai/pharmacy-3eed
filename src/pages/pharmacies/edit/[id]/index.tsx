...
import { getPharmacyById, updatePharmacyById } from 'apiSdk/pharmacies';
...
function PharmacyEditPage() {
  const router = useRouter();
  const id = router.query.id as string;
  const imageUploaderRef = useRef(null);
  const { data, error, isLoading, mutate } = useSWR<PharmacyInterface>(
    () => (id ? `/pharmacies/${id}` : null),
    () => getPharmacyById(id),
  );
  const [formError, setFormError] = useState(null);

  const handleSubmit = async (values: PharmacyInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      await imageUploaderRef.current.handleUpload();
      const updated = await updatePharmacyById(id, values);
      mutate(updated);
      resetForm();
  ...
  const formik = useFormik<PharmacyInterface>({
    initialValues: data,
    ...
    >
      <Box rounded="md">
        ...
        <FormWrapper wrapperProps={{ mb: 3 }}>
          <ImagePicker ref={imageUploaderRef} onChange={handleImageChange} entity="pharmacy" entityId={id} />
        </FormWrapper>
