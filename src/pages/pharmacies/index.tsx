...
import { getPharmacies, deletePharmacyById } from 'apiSdk/pharmacies';
import { PharmacyInterface } from 'interfaces/pharmacy';

type ColumnType = ColumnDef<PharmacyInterface, unknown>;

interface PharmacyListPageProps {
...
export function PharmacyListPage(props: PharmacyListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<PharmacyInterface>>(
    () => `/pharmacies?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: PharmacyInterface) => {
    if (hasAccess('pharmacy', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/pharmacies/view/${row.id}`);
    }
  };
  ...
  const columns: ColumnType[] = [
    ...
    hasAccess('user', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ...
      : null,
    hasAccess('medicine', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          id: 'medicine',
          header: 'Medicine',
          accessorKey: 'medicine',
          cell: ({ row: { original: record } }: any) => record?._count?.medicine,
        }
      : null,
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/pharmacies/view/${record.id}`} passHref legacyBehavior>
                <Button
              ...
              {hasAccess('pharmacy', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                <NextLink href={`/pharmacies/edit/${record.id}`} passHref legacyBehavior>
                  <Button
                    onClick={(e) => e.stopPropagation()}
                    mr={2}
                    padding="0rem 0.5rem"
              ...
              {hasAccess('pharmacy', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
                <IconButton
                  onClick={(e) => {
                    e.stopPropagation();
