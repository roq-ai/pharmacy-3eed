...
const SidebarContent = ({ onClose, ...rest }: SidebarProps) => {
  ...
  const tenantRoles = ['owner', 'pharmacy-manager'];
  const isTenantUser = tenantRoles.some((role) => session?.user?.roles?.includes(role));
  const { data, error, isLoading } = useSWR<PharmacyInterface[]>(
    () => (isTenantUser && session?.user?.tenantId ? `/pharmacies` : null),
    () => getPharmacies({ tenant_id: session?.user?.tenantId }).then(({ data }) => data),
  );
  loading = isLoading;
  ...
  const MockedLinkItems: Array<NavItemPropsInterface> = [
    ...
    {
      name: 'Pharmacies',
      path: isTenantUser ? `/pharmacies/view/${data?.[0]?.id}` : '/pharmacies',
      entity: 'pharmacy',
      service: AccessServiceEnum.PROJECT,
      icon: FiHome,
    },
    { name: 'Customers', path: '/customers', entity: 'customer', service: AccessServiceEnum.PROJECT, icon: FiUsers },
    { name: 'Medicines', path: '/medicines', entity: 'medicine', service: AccessServiceEnum.PROJECT, icon: FiPackage },
    {
      name: 'Purchases',
      path: '/purchases',
      entity: 'purchase',
    ...
    { name: 'Suppliers', path: '/suppliers', entity: 'supplier', service: AccessServiceEnum.PROJECT, icon: FiTruck },
