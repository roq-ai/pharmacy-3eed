  ...
  const fetcher = useCallback(
    async () =>
      getSuppliers({
        ...
        order: params.order,
        searchTermKeys: ['name.contains', 'contact_number.contains', 'address.contains', 'email.contains'],
        ...(params.filters || {}),
      }),
    [params.pageSize, params.pageNumber, params.searchTerm, params.order, params.filters],
  );

  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<SupplierInterface>>(
    () => `/suppliers?params=${JSON.stringify(params)}`,
    fetcher,
  );
