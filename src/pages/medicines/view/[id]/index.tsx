...
import { getMedicineById } from 'apiSdk/medicines';
import { MedicineInterface } from 'interfaces/medicine';
import { PurchaseListPage } from 'pages/purchases';

function MedicineViewPage() {
  const { hasAccess } = useAuthorizationApi();
  const router = useRouter();
  const id = router.query.id as string;
  const { data, error, isLoading, mutate } = useSWR<MedicineInterface>(
            ...
            <FormWrapper wrapperProps={{ border: 'none', gap: 3, p: 0 }}>
              <Flex alignItems="center" w="full" justifyContent={'space-between'}>
                ...
                {hasAccess('medicine', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                  <NextLink href={`/medicines/edit/${id}`} passHref legacyBehavior>
                    <Button
              ...
              >
                ...
                {hasAccess('pharmacy', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                  <FormListItem
                    label="Pharmacy"
                    text={
                      <Link as={NextLink} href={`/pharmacies/view/${data?.pharmacy?.id}`}>
                        {data?.pharmacy?.name}
                      </Link>
                    }
                  />
